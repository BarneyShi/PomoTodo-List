...
//  throws InvalidProgressException if !(0 <= progress <= 100)
    public void setProgress(int progress) throws InvalidProgressException {
        ...
        if (progress < 0 || progress > 100) {
            throw new InvalidProgressException("Progress must 0 <= progress <= 100");
        }
        if (progress == 100) {
            status = Status.DONE;
        }
...
//  throws NegativeInputException if hours < 0
    public void setEstimatedTimeToComplete(int hours) throws NegativeInputException {
        // stub
        if (hours < 0) {
            throw new NegativeInputException("Hours must >= 0");
        }
        etcHours = hours;
    }
